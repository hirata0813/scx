...
struct {
    ...
    __uint(max_entries, 8192);
    __type(key, pid_t);
    __type(value, s32); /* flag: 1 if priority task */
} prev_cpu_list SEC(".maps");
...
void BPF_STRUCT_OPS(priority_enqueue, struct task_struct *p, u64 enq_flags)
{
	...
	// cpu map について，以前用いていた CPU のタスク数を1減らす
	prev = bpf_map_lookup_elem(&prev_cpu_list, &pid);

	// まず，prev_cpu_listから，以前のCPUを取得(エントリがない場合はエントリを作り，取得)
	if (prev == NULL) {
		bpf_map_update_elem(&prev_cpu_list, &pid, &current_cpu, BPF_ANY);	
		prev = bpf_map_lookup_elem(&prev_cpu_list, &pid);
    	}

	// 次に，cpu_task_mapをprevで引いて，該当CPUのポインタをtmpに格納
	if (prev != NULL){
	...
	// prev_cpu_list を更新
	bpf_map_update_elem(&prev_cpu_list, &pid, &cpu, BPF_ANY);	
